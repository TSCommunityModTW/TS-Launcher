...
use crate::util::{app_path, fetch, io};
use super::version::AssetIndex;
...
pub async fn get_asset_objects(asset_index: &AssetIndex, assets_index_id: &str) -> crate::Result<Vec<AssetObjects>> {
    ...
    let asset_objects = fetch::request_json::<McAssetObjects>(&url).await?;

    // TODO: write file
    let asset_path = Path::new(&app_path::get_common_dir_path()).join("assets").join("indexes").join(format!("{}.json", assets_index_id));
    io::write_struct_file(&asset_path, &asset_objects).await?;
...
fn iter_map(objects: &HashMap<String, Object>) -> Vec<AssetObjects> {
    ...
    objects.iter().map(|(_i, object)| {
        ...
        AssetObjects {
            ...
            path: app_path::combine_common_paths_absolute(&Path::new("assets").join("objects"), &relative_path),
